const fi = (function() {
  return {
    ...
    each: function(collection, alertItem) {
      const newCollection = (collection instanceof Array) ? collection.slice() : Object.values(collection)
      
      for (let i = 0; i < newCollection.length; i++) {
        alertItem(newCollection[i])
    ...
    map: function(collection, alertItem) {
      ...
      newArray.push(alertItem(collection[i]))

      return newArray
    },

		reduce: function(items = [], callback = () => {}, acc) {
      let collection = items.slice(0)
